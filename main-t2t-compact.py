...
def init_parser():
    ...
    parser.add_argument('--model', type=str, default='t2t', choices=MODELS)
...
def main(args):
    ...
    model = T2T_ViT(img_size=32, num_classes=100,num_heads=4,depth=12, drop_path_rate=0.1,embed_dim=256).cuda()
    
    prune_mask=[False,False,True,True,False,False,False, False, False, False, False, False]
    
    block_idx = 0
    for layer_idx, layer in enumerate(model.blocks):  # 遍历每一层
        if prune_mask[block_idx]:  # 需要剪枝
                # 用 Identity 替换 Block
            model.blocks[layer_idx] = torch.nn.Identity()  
            print(f"Pruning Block {block_idx} in Layer {layer_idx}")  # 显示剪枝的层
        block_idx += 1
    print(model)
