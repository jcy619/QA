...
#         # 直接删除 Transformer Block 的 5、6 层
#         self.layers = nn.ModuleList([layer for i, layer in enumerate(self.layers) if i not in [4, 5]])

#         # 更新 depth
#         self.num_layers = len(self.layers)  # 确保 depth 和 layers 一致

#         # 重新调整 DropPath（如果有随机深度）
#         self.drop_path = DropPath(stochastic_depth * len(self.layers) / depth) if stochastic_depth > 0 else nn.Identity()
...
class ViT(nn.Module):
    def __init__(self, *, img_size, patch_size, num_classes, dim, depth, heads, mlp_dim_ratio, channels = 3, 
        ...
        # )
        #self.change_dim = nn.Linear(192,256)
        
        self.apply(init_weights)
        self.query_head = QueryHead(num_classes=num_classes,
                                    num_queries=1,
                                    embed_dim=192,
