...
def init_parser():
    ...
    # Data args
    #parser.add_argument('--data_path', default='work/tiny-imagenet-200', type=str, help='dataset path')
    parser.add_argument('--data_path', default='work/SPT_LSA_ViT-main/data', type=str, help='dataset path')
...
def main(args):
    ...
    #Directly remove the first stage, and set the embedding dimension to start from 192.
    depths = [6,4] #2,6,4
    num_heads = [6,12] #3,6,12
    mlp_ratio = 2
    window_size = 4
  
    model=SwinTransformer(img_size=32,embed_dim=192,window_size=window_size,drop_path_rate=0.1,
                          patch_size=2,mlp_ratio=mlp_ratio,depths=depths,num_heads=num_heads,num_classes=100,
                          is_SPT=False, is_LSA=False).cuda()
   #Prune the last block of stage 2.
    prune_mask=[ False,False,False,False,False,True,False,False,False,False] #1
    ...
    for layer in model.layers:
        for i, block in enumerate(layer.blocks):  # 遍历 SwinTransformerBlock
                if prune_mask[block_idx]:  # 如果当前 Block 需要剪枝
                    layer.blocks[i] = nn.Identity()  # 用 Identity 替换该 Block
